...
sys.path.append(r'G:\SRAF\TextualAnalsis\Python')  # Modify to identify path for custom modules
...
MASTER_DICTIONARY_FILE = settings.input_path + r'/LoughranMcDonald_MasterDictionary_2020.csv'
# User defined output file
OUTPUT_FILE = settings.csv_path+'/Parser.csv'
# Setup output
OUTPUT_FIELDS = ['token', 'file size', 'number of words', 'negative', 'positive',
                 'uncertainty', 'litigious', 'strong modal', 'weak modal',
                 'constraining', '# of alphabetic', '# of digits',
                 '# of numbers', 'avg # of syllables per word', 'average word length', 'vocabulary','% negative', '% positive',
                 '% uncertainty', '% litigious', '% strong modal', '% weak modal',
                 '% constraining']                                                                      #Dataframe header

lm_dictionary = LM.load_masterdictionary(MASTER_DICTIONARY_FILE, print_flag=True)                       #Load the dictionary from  LoughranMcDonald_MasterDictionary_XXXX.csv
...
def main():
    ...
    for file in file_list:
        ...
        try:
            with open(file, 'r', encoding='UTF-8', errors='ignore') as f_in:                #Open Dataframe
                doc = f_in.read()
        except:
            e_count+=1
            continue
        ...
        if n_files % 200==0: 
            df2.to_excel(settings.csv_path+'/parser.xlsx')
        #if n_files==50: break
    df2.to_excel(settings.csv_path+'/parser.xlsx')
    ...
    for token in tokens:
        if not token.isdigit() and len(token) > 1 and token in lm_dictionary:       #Check if token is in dictionary
            ...
            if token not in vdictionary:
                vdictionary[token] = 1                                             #Raise counter by one if word fit in category
            if lm_dictionary[token].negative: _odata[3] += 1                       #Negative
            if lm_dictionary[token].positive: _odata[4] += 1                       #Positive
...
if __name__ == '__main__':
    df1= pd.read_csv(settings.csv_path+'/tokencsv.csv', sep=";", index_col=0)            #import filepaths
    file_list=df1['token'].to_list()
    start = dt.datetime.now()
    print(f'\n\n{start.strftime("%c")}\nPROGRAM NAME: {sys.argv[0]}\n')
    df2=main()
    result= pd.merge(df1, df2, on=['token'])                                                    #merge both dataFrame togehter
    result['tone']=(result['positive']-result['negative'])/(result['negative']+result['positive'])  #Calculate Tone based on positive and negative word count
    result.to_excel(settings.csv_path+'/parsingdata.xlsx')                                     #Output result DataFrame
